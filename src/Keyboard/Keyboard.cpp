...
namespace util3ds {
    ...
    void Keyboard::loadFromFile(const std::string &filename) {
        ...
        while (layoutNode != NULL) {
            ...
            if (!layout.name.empty()) {
                ...
                // Load all ToggleButton elements
    ...
    void Keyboard::updateButtonVertices(Button &button) {
        ...
        vertices[3].position = cpp3ds::Vector2f(button.rect.left + button.rect.width,
                                                button.rect.top + button.rect.height);

        vertices[0].texCoords = cpp3ds::Vector2f(state.textureRect.left, state.textureRect.top);
        vertices[1].texCoords = cpp3ds::Vector2f(state.textureRect.left + state.textureRect.width,
                                                 state.textureRect.top);
        vertices[2].texCoords = cpp3ds::Vector2f(state.textureRect.left,
                                                 state.textureRect.top + state.textureRect.height);
        vertices[3].texCoords = cpp3ds::Vector2f(state.textureRect.left + state.textureRect.width,
                                                 state.textureRect.top + state.textureRect.height);
    ...
    void Keyboard::updateVertices() {
        ...
        m_input.text.setPosition(
                static_cast<int>(160.f + style.defaultState.offsetX - m_input.text.getGlobalBounds().width / 2),
                m_input.text.getPosition().y);

        // Update cursor position
        cpp3ds::Text dummyText(
                m_input.data.substring(m_input.data.getSize() - m_input.cursorPosition, m_input.cursorPosition),
                *m_input.text.getFont(),
                m_input.text.getCharacterSize());
        m_cursor.setPosition(
                m_input.text.getPosition().x + m_input.text.getGlobalBounds().width - dummyText.getLocalBounds().width,
